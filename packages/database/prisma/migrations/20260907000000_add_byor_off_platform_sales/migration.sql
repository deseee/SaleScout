...
-- NOTE: This SQL was already applied to the production Railway database on 2026-09-06 via `prisma db execute`
-- (not through this migration file, since it did not exist yet). This file exists so CI's fresh test database
-- and any future local/fresh environment can pick up the same change through the normal `prisma migrate deploy`
-- mechanism. Production's _prisma_migrations table must be reconciled separately via
-- `prisma migrate resolve --applied 20260907000000_add_byor_off_platform_sales` (bookkeeping only, no SQL executed)
-- so a future `migrate deploy` against Railway does not try to re-run this file and fail on "already exists".
