      ...
      <main className="container mx-auto px-4 py-8">
                  ...
                  {bids.map((bid) => (
                    <tr key={bid.id}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <Link href={`/items/${bid.itemId}`} className="text-blue-600 hover:text-blue-900">
                          View Item
                        </Link>
                      </td>
                    </tr>
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          ...
          <div className="mb-6 p-4 bg-blue-50 rounded-lg">
            <h3 className="font-medium text-blue-800 mb-2">Your Referral Link</h3>
            <div className="flex">
              <input
                type="text"
                readOnly
                value={`${window.location.origin}/register?ref=${user.referralCode}`}
                className="flex-grow px-3 py-2 border border-gray-300 rounded-l-md text-gray-900 bg-white"
              />
              <button
                onClick={() => {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="border rounded-lg p-4">
              <h3 className="font-bold text-lg mb-2">Recent Referrals</h3>
              {referrals.length === 0 ? (
                ...
                <ul className="divide-y">
                  {referrals.slice(0, 3).map((referral) => (
                    <li key={referral.id} className="py-2">
                      <div className="font-medium">{referral.referredUser.name || referral.referredUser.email}</div>
