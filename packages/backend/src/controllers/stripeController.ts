...
export const createCartCheckoutSession = async (req: AuthRequest, res: Response) => {
  try {
    ...
    // Security fix (findasale-hacker adversarial pass, 2026-09-10 -- mirrors the identical
    // fix just applied to squarePaymentController.ts's createSquareCartPayment): the prior
    ...
    // Stripe account, stock-decremented, and given a Purchase row stamped with a saleId it
    // never belonged to -- a real cross-tenant charge/misattribution, not merely a validation
    // gap. Fix: every item must have a NON-NULL saleId, and all of them must be the SAME
    // saleId -- no more filtering nulls out of the uniqueness check.
